...
---
Важно: используются потенциометры (не энкодеры) и включён режим “pickup” (soft takeover).
При переключении режима ручки не начинают менять параметр сразу. Сначала нужно “поймать” сохранённую позицию ручки для этого режима: ручка начинает работать только когда её физическое положение приблизится к последнему сохранённому положению (примерно в пределах ±1–2% хода).
При первом входе в режим сохранённая позиция равна 0, поэтому иногда нужно повернуть ручку почти до упора влево, чтобы управление “подхватилось”.

После того как ты однажды покрутил ручку в режиме, её позиция запоминается для этого режима, и в следующий раз “подхват” будет уже около последнего положения, а не около нуля.
---
