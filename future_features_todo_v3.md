...
## Patch should store Tune state (future)

**Goal:** make `TUNE` + `TRANSPOSE` fully recallable by loading a patch (no extra “device settings” file).

### Data to store in patch JSON (saved only on explicit Save)
- `transpose`: integer semitone offset (default `0` if missing)
- `tune_note_int`: array of 4 integers (per pad MIDI note number, default = base `touch_midi_notes`)
- `tune_fine_cents`: array of 4 integers (per pad fine cents `-100..+100`, default `0`)

### Apply rules
- **No autosave.** Values are written only when user performs patch Save.
- On patch Load:
  - If any field is missing → fall back to defaults (transpose=0, tune to base notes, fine=0).
  - Update runtime tune arrays immediately (so touch preview + latch notes use tuned values).
