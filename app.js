/* file: app.js (umd-v1)
 * Requires:
 * - deps/ffmpeg.min.js  (defines global FFmpegWASM)
 * - deps/util.min.js    (defines global FFmpegUtil with fetchFile + toBlobURL)
 * - deps/jszip.min.js   (defines global JSZip)
 * - deps/ffmpeg-core.js + deps/ffmpeg-core.wasm (+ optional worker.js)
 *
 * This avoids ESM dynamic-import issues on GitHub Pages.
 */
(() => {
  const UI = {
  ...
  async function selfCheckDeps() {
    ...
    for (const url of urls) {
      try {
        ...
        log(`${r.ok ? "✅" : "❌"} ${url} -> ${r.status}`);
      } catch (e) {
        log(`❌ ${url} -> ${e?.message || String(e)}`);
      }
    }

    log(`Globals: FFmpegWASM=${!!window.FFmpegWASM}, FFmpegUtil=${!!window.FFmpegUtil}, JSZip=${!!window.JSZip}`);
    log("Self-check done.");
  }
  ...
  async function ensureFFmpeg() {
    ...
    if (!window.FFmpegWASM) throw new Error("FFmpegWASM global not found. deps/ffmpeg.min.js missing?");
    if (!window.FFmpegUtil) throw new Error("FFmpegUtil global not found. deps/util.min.js missing?");
    ...
    // For UMD version of @ffmpeg/core you may also have ffmpeg-core.worker.js;
    // We don't require it for single-thread setup.
  ...
  function writeWavPcm16Mono(samples, sampleRate = 44100) {
    ...
    const putU32 = (off, v) => dv.setUint32(off, v, true);
    const putU16 = (off, v) => dv.setUint16(off, v, true);

    putStr(0, "RIFF"); putU32(4, riffSize); putStr(8, "WAVE");
    putStr(12, "fmt "); putU32(16, 16); putU16(20, 1); putU16(22, numChannels);
    putU32(24, sampleRate); putU32(28, byteRate); putU16(32, blockAlign); putU16(34, bitsPerSample);
    putStr(36, "data"); putU32(40, dataSize);
