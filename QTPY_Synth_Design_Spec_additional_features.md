...
**Purpose:** fastest way to store/recall 4 slots during play.

- **Hold KEY 2 seconds → ARM LOAD**
  - While armed: Touch pad **1–4** selects slot **1–4** to **Load** immediately.
- **Hold KEY 5 seconds → ARM SAVE**
  - While armed: Touch pad **1–4** selects slot **1–4** to **Save** current patch immediately.
- Cancel arming:
  - Release KEY without pad selection → cancel.
  - Optional: timeout (e.g. 5s) → cancel.

**Notes**
- This is separate from the “Setup Menu”.
- This adds timing thresholds; original code only had “key_held boolean”.
- Must provide clear feedback (display text or LED blink pattern).

---

### B) Setup Menu (Screen #2)
**Entry:** Hold KEY **10 seconds** from main screen → enter Setup Menu.  
